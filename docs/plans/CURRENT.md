...
- 阶段：P1 `AI 语音输入法`
- 当前 focus：基于第一轮 Settings 壳体验收，收口窗口一致性、错误控件语义、菜单栏入口与品牌图形
- 状态：`In Progress`
...
4. 重画 logo，改为更明确的麦克风语义
5. 保持现有设置行为、本地化和已有功能不回归
6. 同步 live plan、测试与人工验收
...
4. 核查 menu bar icon 入口
5. 重画麦克风方向 logo，并接入 app / menu bar 资产
6. 跑测试 / build，并做一轮人工界面验收
...
- 品牌图形已切到麦克风方向，menu bar 图标已改成更高识别度的麦克风语义
- 新的 Settings 视觉层级已落地第一版，当前人工验收已明确收口方向：
  窗口一致性、动作语义、menu bar icon 可见性、logo 重画
