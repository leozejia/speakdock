...
- `Qwen3.5-0.8B` 是当前最像默认路线的轻量候选
- `Qwen3.5-2B` 是不跨进 `4B` 的上限试探
- `Gemma 3 1B` 是结构更简单的替代对照组

执行规则：

- 先只跑 `Qwen3.5-0.8B`
- 如果第一候选在闸门 A 就失败，先修样本或 prompt，不继续扩大候选池
- 只有第一候选达到 `watchlist` 以上，才继续测第二个
