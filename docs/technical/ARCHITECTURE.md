...
- P1 热路径不依赖本地 ASR 模型
- 下一阶段优先评估 `Qwen3-ASR-0.6B via MLX` 这条端侧 ASR 候选路线
...
- 轻量本地模型负责 `Workspace Refine`
- 确定性模板保证输出可控
- 置信度门控决定是否保守直出，还是进入更重的 `Workspace Refine`
...
- 以后若加入本地整理模型，也挂在 `ASR Post-Correction` 或 `Workspace Refine` 之后的对应层里，不能反向污染默认热路径
...
- `asr`
  ...
  - 下一阶段优先评估 `Qwen3-ASR-0.6B via MLX` 作为本地 ASR 候选
  - Whisper（whisper.cpp / MLX-Whisper）作为备选
  - Apple Speech 角色收窄为：`Streaming Preview`（用户视觉反馈）+ 零配置 fallback（未下载本地模型时兜底）
  - 本地 ASR 是否通过 sidecar、是否常驻、如何打包，不在当前阶段锁死
  - Settings UI 后续需要 ASR Engine 配置区：Provider / Model path / Streaming preview toggle
...
- `asr_post_correction`
  ...
  - 如果后续本地 ASR 候选的实际词准已经足够高，该层的必要性会下降；当前 seam 保留但继续默认关闭，待端侧 ASR 评测后根据真实错误样本重新评估
  - 长期优先端侧小模型或本地服务，云接口先作为可选验证路径
