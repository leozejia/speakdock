...
- `TermDictionary`
  - 负责“这个词到底是什么”
  - 它是会影响后续 `Clean / ASR 后处理 / Refine 上下文` 的 active 词级事实层

- `Word Correction`
  - 不是独立对外能力
  - 它只是 `TermDictionary` 下面的被动学习机制
  - 它只观察词或短语级别的替换，不处理整句
  - 它的目标是让词典越来越准，不是替代 `Refine`
...
- `ASR` 负责实时转录
- 术语词典负责名称、产品名、技术词汇
- `ASR Correction` 负责“转写文本已经存在，但词不够准”
- 轻量本地模型负责 workspace 级整理
- 确定性模板保证输出可控
...
- 包括：
  ...
  - 口头禅清理
  - 术语词典修正
- 当前默认热路径也不引入模型型 `ASR Correction`
- 未来如果加入 `ASR Correction`，它也必须是可选层，并且失败时直接回退到 `Clean`
- `Refine` 是可选增强
- 录音阶段只按 `Clean` 结果追加到工作区
- 没开 refine 时，发送阶段直接提交当前工作区
- 开了 refine 且配置完整时，只在发送前或用户手动整理时进入 workspace 级整理
- 以后若加入本地整理模型，也挂在 `ASR Correction` 或 `Refine` 之后的对应层里，不能反向污染默认热路径
