...
1. SpeakDock 记录本次工作区里“最后一次由 SpeakDock 渲染的文本”和用户最终保留文本之间的差异摘要
...
- 如果工作区还没经过 `Refine`，这个“最后一次渲染的文本”通常就等于当前工作区的 `raw_context`
- 如果工作区已经过 `Refine`，词典候选必须以整理后的当前可见文本为基线，不能再回退到更早的口语基线，否则会把整理改写也误当成术语差异
- v1 只在当前目标仍然可观测时尝试提取这份“最终保留文本”
- paste-only fallback 或不可读回文本的目标，不承诺生成候选；此时系统应该保守跳过，而不是猜测
