...
APP_DIR="$ROOT_DIR/.build/$CONFIGURATION/SpeakDockTestHost.app"
...
print -u2 -- "Building SpeakDockTestHost ($CONFIGURATION)..."
swift build --package-path "$ROOT_DIR" -c "$CONFIGURATION" --product SpeakDockTestHost >&2

BUILD_BIN_DIR="$(swift build --package-path "$ROOT_DIR" -c "$CONFIGURATION" --show-bin-path 2>/dev/null)"
EXECUTABLE_PATH="$BUILD_BIN_DIR/SpeakDockTestHost"

rm -rf "$APP_DIR"
mkdir -p "$MACOS_DIR" "$RESOURCES_DIR"

cp "$ROOT_DIR/Sources/SpeakDockTestHost/Resources/Info.plist" "$CONTENTS_DIR/Info.plist"
cp "$EXECUTABLE_PATH" "$MACOS_DIR/SpeakDockTestHost"
...
print -u2 -- "Test host bundle ready: $APP_DIR"
print -- "$APP_DIR"
